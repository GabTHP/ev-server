...
export default class UtilsService {
  ...
  public static async exportToPDFTransaction(req: Request, res: Response, attachmentName: string, filteredRequest: any,
    handleGetData: (req: Request, filteredRequest: any) => Promise<DataResult<any>>,
    handleConvertToPDFTransaction: (req: Request, pdfDocument: PDFKit.PDFDocument, data: any[]) => Promise<string>): Promise<void> {
  // Override
  req.query.Limit = Constants.EXPORT_PDF_PAGE_SIZE.toString();
  // Set the attachment name
  res.attachment(attachmentName);
  ...
  do {
    ...
    data = await handleGetData(req, filteredRequest);
    // Transform data
    await handleConvertToPDFTransaction(req, pdfDocument, data.result);
    // Next page
    skip += Constants.EXPORT_PAGE_SIZE;
  } while (skip < count);
  // Finish
